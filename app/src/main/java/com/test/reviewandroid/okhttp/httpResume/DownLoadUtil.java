...
public class DownLoadUtil {
    ...
    public void download(final Context context, @NonNull final String downloadUrl, @NonNull final String filePath,
        ...
        Log.d(TAG, "方法：download: properties路径" + configFile);
        try {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    int code = conn.getResponseCode();//得到返回的状态码
                    if (code == 200) {
                        ...
                        for (int threadId = 0; threadId < THREAD_NUM; threadId++) {
                            long startL = threadId * blockSize, endL = (threadId + 1) * blockSize;
                            Object state = pro.getProperty(dFile.getName() + "_state_" + threadId);
                            if (state != null && Integer.parseInt(state + "") == 1) {  //该线程已经完成
                                mCurrentLocation += endL - startL;
                                Log.d(TAG, "++++++++++ 线程_" + threadId + "_已经下载完成 ++++++++++");
                                mCompleteThreadNum++;
                            ...
                            //分配下载位置
                            Object record = pro.getProperty(dFile.getName() + "_record_" + threadId);
                            if (!newTask && record != null && Long.parseLong(record + "") > 0) {       //如果有记录，则恢复下载
                                Long r = Long.parseLong(record + "");
                                mCurrentLocation += r - startL;
                                Log.d(TAG, "++++++++++ 线程_" + threadId + "_恢复下载 ++++++++++");
                            ...
                            DownloadEntity entity = new DownloadEntity(context, fileLength, downloadUrl, dFile, threadId, startL, endL);
                            DownLoadTask task = new DownLoadTask(entity);
                            tasks.put(threadId, new Thread(task));
                        }
